 ...
 * @Last Modified time: 2017-12-18 14:02:45
*/
...
const NOTIFY_URL = 'http://127.0.0.1:3001/api/order/paynotify'
// 支付成功之后跳转页面
const RETURNURL = 'http://127.0.0.1:3001/api/order/payreturn'
// const RETURNURL = `${config.vueUrl}/#/user/order/order?state=1`
...
class Alipay {
    ...
    signVerify (opts) {
        ...
        tempArray.forEach((item) => {
            tempStr += `${item}=${opts.query[item]}&`
        })
        tempStr = tempStr.slice(0, -1)
        return AlipayUtils.signVerify(tempStr, sign, ALIPAY_PUBLIC_KEY_PEM)
    }
